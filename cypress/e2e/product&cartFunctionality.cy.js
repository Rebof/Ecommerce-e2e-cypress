...
describe("Product and cart functionality check", () => {
  ...
  it.only("Adding product to the cart and verifying total price", () => {
  const quantity = 1;
  ...
  cy.get(".features_items .col-sm-4").eq(2).within(() => {
    cy.get(".productinfo p")
      .invoke("text")
      .then((text) => {
  ...
  cy.then(() => {
    ...
    const parsedFirstProdPrice = parseFloat((firstProdPrice + '').replace(/[^0-9.]/g, ''));
    const parsedSecondProdPrice = parseFloat((secondProdPrice + '').replace(/[^0-9.]/g, ''));
    const parsedCartFirstProdTotal = parseFloat((cartFirstProdTotal + '').replace(/[^0-9.]/g, ''));
    const parsedCartSecondProdTotal = parseFloat((cartSecondProdTotal + '').replace(/[^0-9.]/g, ''));
